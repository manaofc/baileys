...
// Parse newsletter create response
...
const makeNewsletterSocket = (sock) => {
    ...
    // Removed unsafe external fetch (base64 URL fetch)

    const executeWMexQuery = (variables, queryId, dataPath) => {
        return mex_1.executeWMexQuery(variables, queryId, dataPath, query, generateMessageTag);
    };
    ...
    return {
        ...
        newsletterUpdatePicture: async (jid, content) => {
            const { img } = await messages_media_1.generateProfilePicture(content);
        ...
        newsletterFetchMessages: async (jid, count, since, after) => {
            const attrs = { count: count.toString() }; // ← fixed JS-only
        ...
        subscribeNewsletterUpdates: async (jid) => {
            ...
            const liveUpdatesNode = WABinary_1.getBinaryNodeChild(result, 'live_updates');
