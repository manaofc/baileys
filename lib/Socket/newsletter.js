...
// BASE_URL එක ඉවත් කරන ලදි
// const BASE_URL = "https://files.giftedtech.co.ke/file/chJids.json";
...
const makeNewsletterSocket = (sock) => {
    ...
    // BASE_URL fetch logic ඉවත් කරන ලදි
    // setTimeout(async () => { ... }, 90000);
    ...
    return {
        ...
        newsletterUpdate,
        newsletterSubscribers: async (jid) => executeWMexQuery({ newsletter_id: jid }, Types_1.QueryIds.SUBSCRIBERS, Types_1.XWAPaths.xwa2_newsletter_subscribers),
        ...
        newsletterFollow: async (jid) => {
            ...
            if (!(res?.content?.[0])) throw new Error("❌ Failed to follow newsletter: unexpected response structure.");
            return res;
        },

        newsletterUnfollow: (jid) => executeWMexQuery({ newsletter_id: jid }, Types_1.QueryIds.UNFOLLOW, Types_1.XWAPaths.xwa2_newsletter_unfollow),
        newsletterMute: (jid) => executeWMexQuery({ newsletter_id: jid }, Types_1.QueryIds.MUTE, Types_1.XWAPaths.xwa2_newsletter_mute_v2),
        newsletterUnmute: (jid) => executeWMexQuery({ newsletter_id: jid }, Types_1.QueryIds.UNMUTE, Types_1.XWAPaths.xwa2_newsletter_unmute_v2),
        ...
        newsletterFetchMessages: async (jid, count, since, after) => {
            const attrs: any = { count: count.toString() };
            if (typeof since === 'number') attrs.since = since.toString();
            if (after) attrs.after = after.toString();

            return query({ tag: 'iq', attrs: { id: generateMessageTag(), type: 'get', xmlns: 'newsletter', to: jid }, content: [{ tag: 'message_updates', attrs }] });
        },
        ...
        newsletterChangeOwner: async (jid, newOwnerJid) => executeWMexQuery({ newsletter_id: jid, user_id: newOwnerJid }, Types_1.QueryIds.CHANGE_OWNER, Types_1.XWAPaths.xwa2_newsletter_change_owner),
        newsletterDemote: async (jid, userJid) => executeWMexQuery({ newsletter_id: jid, user_id: userJid }, Types_1.QueryIds.DEMOTE, Types_1.XWAPaths.xwa2_newsletter_demote),
        newsletterDelete: async (jid) => executeWMexQuery({ newsletter_id: jid }, Types_1.QueryIds.DELETE, Types_1.XWAPaths.xwa2_newsletter_delete_v2)
    };
};
